...
function load_bbcode_template()
{
	...
	$tpl_filename = './lib/BBCode/bbcode.tpl';
...
function bbencode_second_pass($text, $uid)
{
	...
	// unordered lists
	$text = str_replace("[list:$uid]", $bbcode_tpl['ulist_open']."\n", $text);
	// li tags
	$text = str_replace("[*:$uid]", $bbcode_tpl['listitem_open'], $text);
	$text = preg_replace('@'.$bbcode_tpl['listitem_open'].' (.*)<br />\n@i', $bbcode_tpl['listitem_open'].'\1'.$bbcode_tpl['listitem_close']."\n", str_replace("\r", '',$text));
	// ending tags
	$text = str_replace("[/list:u:$uid]", $bbcode_tpl['ulist_close'], $text);
	$text = str_replace("[/list:o:$uid]", $bbcode_tpl['olist_close'], $text);
	// Ordered lists
	$text = preg_replace("/\[list=([a1]):$uid\]/si", $bbcode_tpl['olist_open']."\n", $text);
