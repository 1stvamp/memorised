...
class twgrowler (twnotifier) :
    ...
    def setup_notification_agent(self) :
        ...
        try :
            self.__growl__ = GrowlNotifier(applicationName="twgrowler",
                                           notifications=["twgrowl"],
                                           defaultNotifications=["twgrowl"])
        except Exception, e :
            self.error("Failed to create notification agent, %s" % e)
            return False

        try :
            self.__growl__.register()
        except Exception, e :
            self.error("Failed to register notification agent, %s" % e)
            return False
    ...
    def notify (self, tw) :
        ...
        title  = tw['who']
        image_file  = tw['looks']
        desc   = self.format_twitter_message(tw)
        try :
            gicon  = Growl.Image.imageFromPath(image_file)
        except Exception, e :
            self.error("Couldn't load image, %s" % e)
        
        try :
            self.__growl__.notify("twgrowl", title, desc, gicon, True)
        except Exception, e :
...
if __name__ == "__main__" :
    ...
    open(".twnotifier.log", "w").write(procParams + "\n")
